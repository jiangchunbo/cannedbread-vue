<template>
  <div class="heroic-expeditions-container">
    ...
    <el-table :data="filterEquipmentData" height="calc(100% - 60px)">
      <el-table-column prop="attribute" label="装备" width="400">
        <template slot-scope="scope">
          <el-image
            style="width: 350px"
            :src="`https://jiangchunbo-1302516612.cos.ap-shanghai.myqcloud.com/war3/英雄的远征/${scope.row.category}/${scope.row.name}/属性.png`"
            :preview-src-list="[
              `https://jiangchunbo-1302516612.cos.ap-shanghai.myqcloud.com/war3/英雄的远征/${scope.row.category}/${scope.row.name}/属性.png`,
            ]"
          />
        </template>
      </el-table-column>
      <el-table-column prop="combinationRule" label="获取方式" width="500">
        <template slot-scope="scope">
          <el-image
            ...
            :preview-src-list="[
              `https://jiangchunbo-1302516612.cos.ap-shanghai.myqcloud.com/war3/英雄的远征/${scope.row.category}/${scope.row.name}/获取方式.png`,
            ]"
          />
          <span style="white-space: pre-line" v-else>{{
            scope.row.combinationRule
          }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="combinationRule" label="备注" width="300">
        <template slot-scope="scope">
...
export default {
  ...
  computed: {
    filterEquipmentData() {
      const search = this.form.level != null || this.form.category != null;
        ...
        return (
          (!level ||
            (equipment.level &&
              ...
              level &&
              level !== "" &&
              equipment.level.indexOf(level) !== -1)) &&
          (!category ||
            (equipment.category &&
              equipment.category !== "" &&
              category &&
              category !== "" &&
              equipment.category.indexOf(category) !== -1))
        );
      });
      filterEquipmentData.sort((e1, e2) => {
        ...
        if (e1.level && e2.level) {
          ...
          if (qualityValue[quality1] !== qualityValue[quality2]) {
            return qualityValue[quality1] < qualityValue[quality2] ? 1 : -1;
          }
          if (score1 !== score2) {
        ...
        if (e1.primeAttribute && e2.primeAttribute) {
          return (
            orderedPrimeAttribute[e1.primeAttribute] -
            orderedPrimeAttribute[e2.primeAttribute]
          );
        }
        if (!e1.primeAttribute || !e2.primeAttribute) {
